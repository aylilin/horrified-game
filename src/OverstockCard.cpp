...
{
    if (!cardTexture.loadFromFile("../build/Perk_Cards/Overstock.png"))
    {
    ...
    font.loadFromFile("../build/ManufacturingConsent-Regular.ttf");
...
void OverstockCard::apply(Hero& currentHero , std::vector<Hero*>& allHeroes , std::vector<Monster*>& monsters , ItemBag& bag , Map& map , bool& skipMonsterPhase)
{

    currentHero.addExtraActions(2);
    currentHero.resetActions();
    std::cout << "Added 2 extra actions to hero: " << currentHero.get_name() << "\n";
    ...
    for (Hero* hero : allHeroes)
    {
        ...
        if (!items.empty())
        {
            ...
            std::cout << "Item placed at " << hero->get_location() << "\n";
        } else {
...
void OverstockCard::handleClick(sf::Vector2f mousePos , Hero& currentHero , std::vector<Hero*>& allHeroes , std::vector<Monster*>& monsters , ItemBag& bag , Map& map , bool& skipMonsterPhase)
{
